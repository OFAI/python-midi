        ...
        for t in eventTimes:
            ...
            for ch, evs in onOffsByChannel.items():
                ...
                for note, v in onOffsByNote.items():
                    ...
                    onOff = partition(
                        lambda x: isinstance(x, NoteOnEvent) and x.velocity > 0, v)
                    ons = onOff.get(True, [])
                    ...
                    if nOffs >= 0:  # there's an off for every 0vel On
                        ...
                        if isSounding:
                            on = sounding[(ch, note)]
                            acc.append(
                                MidiNote(on, onsZeroVel[0]))  # any off will do
                    ...
                    if nOffs > 0:
                        if isSounding:
                            on = sounding[(ch, note)]
                            acc.append(
                                MidiNote(on, offs[0]))  # any off will do
                            del sounding[(ch, note)]
                            isSounding = False
                        else:
    ...
    def homophonic_slices(self):
        ...
        for t in times:
            if not sounding == {}:
                ...
                prev_time = t
            onsOffs = partition(
                lambda x: isinstance(x, NoteOnEvent), onoffs[t])
            ...
            # onOffSnd =
            # list(set.intersection(set.intersection(ons,offs),snds))
            ...
            if len(onSnd) > 1:
                ...
                print(onSnd)
            if len(off) > 1:
                print('Warning, ignoring unexpected %s note off at time %d' %
                      (off, t))
...
def convert_midifile_to_type_0(input_filename, output_filename):
    ...
    """
    convert_midi_to_type_0(
        MidiFile(input_filename)).write_file(output_filename)
...
class _MidiHandler(MidiOutStream):
    ...
    def continuous_controller(self, channel, controller, value):
        channel += 1
        self.events.append(
            ControllerEvent(self.abs_time(), channel, controller, value))
    ...
    def note_on(self, channel=1, note=0x40, velocity=0x40):
        ...
        if self.zero_vel_on_is_off and velocity == 0:
            ...
            self.events.append(
                NoteOnEvent(self.abs_time(), channel, note, velocity))

    def note_off(self, channel=1, note=0x40, velocity=0x40):
        channel += 1
        self.events.append(
            NoteOffEvent(self.abs_time(), channel, note, velocity))

    def header(self, format=0, nTracks=1, division=480):
        self.midiFile.header = MidiHeader(
            format=format, number_of_tracks=nTracks,
